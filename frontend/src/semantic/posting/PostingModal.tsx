    ...
    if (modifyMode)
        return (
            <Modal id={'posting-form'} as={Form} open={visible} style={{textAlign : "center"}} onSubmit={handleSubmit}>
                <Modal.Content image>
                    <Image wrapped src={`http://localhost:8080/${profileImg}`} fluid/>
                </Modal.Content>
                <Modal.Content>
                    <Modal.Description>
    ...
    if (!modifyMode)
        return (
            <Modal open={visible} style={{textAlign : "center"}}>
                <Modal.Content image>
                    {currentPosting.postingAttach !== 'default.png' ? <Image wrapped src={`http://localhost:8080/${currentPosting.postingAttach}`}fluid/> : null}
                </Modal.Content>
                <Modal.Content
                    style={{
                        overflow: "auto",
                ...
                <Modal.Header>
                    ...
                    <Label style={{width : "100%"}} onClick={() => handleProfile(currentWriter.username)} as={'a'} size={"massive"}>
                        <Image src={`http://localhost:8080/${currentWriter.profileImg}`} avatar/>
